...
import { LoggerConfigReader } from './app/configReader/LoggerConfigReader';
...
function entry() {
  ...
  const loggerConfig = new LoggerConfigReader("D:\\project\\homework\\waterball-design-pattern-course-2\\4.4-H\\source\\logger.config.json")
  loggerConfig.initializeLoggers().then(() => {
    const game = new Game();
    game.start();
  });
}
